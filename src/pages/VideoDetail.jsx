...
import { FiThumbsUp, FiShare2, FiPlusSquare, FiLock } from "react-icons/fi";
...
const VideoDetail = () => {
  ...
  useEffect(() => {
    const fetchVideoDetails = async () => {
      try {
        ...
        setError(err.response?.data?.message || "Failed to load corporate media");
      } finally {
        setLoading(false);
      }
    };
  ...
  // 🚨 THE ENTERPRISE SECURITY SCREEN
  if (error || !video) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-zinc-400 gap-4">
        <div className="w-20 h-20 bg-red-500/10 border border-red-500/20 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(239,68,68,0.15)]">
          <FiLock className="text-4xl text-red-500" />
        </div>
        <h2 className="text-2xl font-bold text-white tracking-tight">Access Denied</h2>
        <p className="text-zinc-400 max-w-md text-center leading-relaxed">
          {error || "You do not have the required corporate clearance to view this media, or your access has been revoked by the organization."}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 flex flex-col xl:flex-row gap-8">
      ...
      <div className="flex-1">
        ...
        {/* 🚨 REPLACED STANDARD VIDEO WITH SMART TRACKER */}
